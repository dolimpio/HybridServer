...
public class ServiceThread implements Runnable {
    ...
    public ServiceThread(Socket clientSocket, DAODBHTML daoHTML, DAODBXML daoXML, DAODBXSLT daoXSLT, DAODBXSD daoXSD,
        ...
        xmlController = new XMLController(daoXML, daoXSLT, daoXSD, servers);
    ...
    public void run() {
        ...
        try {
            ...
            try {
                ...
                System.out.println("RESOURCE chain: " + request.getResourceChain());
                System.out.println("RESOURCE name: " + request.getResourceName());
                ...
                if (request.getResourceChain().equals("/")) {
                    ...
                    String pageContent = "Hybrid Server => Miranda Carou Laino, David Olimpio Silva || Esta vez no nos ha dado tiempo a poner citas...";
                    String welcomePage = "<html><head> <title>Root Page</title> </head>"
                            + "<body>" + "<span>" + pageContent + "</span>" + "</body></html>";
                    response.setContent(welcomePage);
                    response.putParameter("Content-Type", "text/html");

                } else if (!request.getResourceChain().contains("html") && !request.getResourceChain().contains("xml")
